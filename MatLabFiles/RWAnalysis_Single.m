classdef RWAnalysis_Single < handle
    ...
    %
    % Need this for everything - only trials with constant vel versus change in vel (add acceleration)
    % Correlation confusion matrix
    % Correct/Incorrect turns gamma timing with low freq suppression
    % Phase amp coupling 
    % Every KDE peak as an option
    % KDE peaks that don't have a speed change
    % Comparing KDE to expert rater (how much lag?)
    % Ratio between before/after in specgram power for gui
    ...
    % Only pick one electrode per participant in AntHipp to prevent double dipping
    ...
    methods %initialization, calculations, etc
        ...
        function obj = RWAnalysis_Single(varargin) %constructor (runs when object is created for the first time)
            obj.parseInputs(varargin{:});

            obj.RootDir = 'C:\Users\burke\OneDrive\Desktop\NeuroIOT\RW1\SingleRw1\';
            obj.AnalysisFile = fullfile(obj.RootDir,'RWAnalysis_Single.mat');
            obj.WinSegSec = 2;
            obj.WinTransSec = 12;
        ...
        function p = parseInputs(obj,varargin)
            ...
            p.regiontype = ''; %'AntHipp','LatTemp','Ent+Peri','PostHipp+Para','All Chans'
            ...
            p.veltype = ''; %High Change, Low Change
                ...
                if ~ismember(WalkNum,obj.DisabledWalks{obj.PatientIdx})
                    ...
                    if isfile(Fname)
                        ...
                        if isfield(SS,'ntp_kde')
                            ...
                            %KDEPeakHigh, KDEPeakLow, KDETrough
        ...
        function getMultTransData(obj,varargin)
            ...
            fprintf('This is the length of patient list: %d\n', length(obj.PatientList));
           
            for k=1:length(obj.PatientList)
                disp(k);
                obj.loadData('PatientIdx',k);
                disp("Got here and then obj was called");
                obj.parseTransData;
                disp("This line was reached!");
        ...
        function [MT,dd,dx] = filterMultTransData(obj,varargin)
            ...
            regiontype = p.regiontype; %'AntHipp','LatTemp','Ent+Peri','PostHipp+Para','All Chans'
            ...
            regionlist = {'AntHipp','LatTemp','Ent+Peri','PostHipp+Para','All Chans'}; %1=AntHipp, 2=LatTemp, 3=Ent+Peri, 4=PostHipp+Para (set in loadData)
            ...
            dd_idx = obj.MultTrans.DT_idx(transidx); % Sample number/id of event     
            ...
            if ~isempty(veltype)
                ...
                t_idx = t>-5 & t<5; %only focus on change in vel for +-5sec
                mdx = median(dx);
                pk_min = min(dx(t_idx,:));
                ...
                mid_idx = round(length(sidx)/2);
                
                switch veltype
                    case 'High Change' %keep highest negative numbers
                        idx = sidx(mid_idx+1:length(sidx)); %discard index
                    case 'Low Change'
                        idx = sidx(1:mid_idx);
                end
                dd(:,idx) = [];
        ...
        function [pwr,tsec,freq] = getFilteredMultTransData(obj,MT,tsamp,varargin)
            ...
            for k=1:size(uPtWkCh,1)
                ...
                wk = uPtWkCh.walk(k);
                ch = uPtWkCh.chan(k);
                didx = round(mt.dataidx/10)+tsamp; %factor 10 downsample (getMultTransData)
                ridx = any(didx<1,2)|any(didx>mt.nsamp(1),2); didx(ridx,:) = []; idx(ridx) = []; %remove any windows that are outside data range
                dd = obj.MultTrans.MultWTable{pt}{wk}(:,:,ch); %time x freq x chan
                dd = reshape(dd(didx,:),size(didx,1),size(didx,2),nfreq);
                pwr(idx,:,:) = dd; %trial x time x freq
            end
            pwr = permute(pwr,[2,3,1]); %time x freq x trial
        end

        function dispCategories(obj,varargin)
            regionlist = {'AntHipp','LatTemp','Ent+Peri','PostHipp+Para','All Chans'};
            walklist = {'First Walks','Last Walks','Stop Walks','Go Walks','All Walks'};
        ...
        function loadMultData(obj,varargin)
            if isfile(obj.AnalysisFile) || isfile('RWAnalysis_Single.mat')
                if isfile('RWAnalysis_Single.mat')
                    analysisfile = 'RWAnalysis_Single.mat';
                else
...
%----------------------------------------------------------------------
%----------------------------------------------------------------------
function varargout = plotSingleTransSpecGramPerm(obj,  StartingSampleID, varargin)
    % Plot specgram of a single transition for the specified parameters.
    ...
    correctiontype = p.correctiontype; % cluster, pixel (if empty, cluster correction is skipped)
    ...
    % Select only one occurrence
    if isempty(MT)
        error('No data found for the specified parameters.');
    end
    MT = MT(1, :); % Select the first occurrence

    % Manual Change can be used to tell the program where EXACTLY to start
    % at base 0 sampling rate. Next we need to alter the before and after.
    MT.dataidx = StartingSampleID;

    disp(MT.dataidx);

    
   
    [pwr, tsec, freq] = obj.getFilteredMultTransData(MT, tsamp); % raw power for the selected trial (time x freq x trial)
    ...
    freq_idx_4_8Hz = freq >= 4 & freq <= 8;
    avg_db_4_8Hz = mean(npwr(:, freq_idx_4_8Hz), 2); % Average dB values in the 4-8 Hz range across the time axis

    % Standardize the average dB values to the range -1 to 1
    avg_db_4_8Hz_standardized = (avg_db_4_8Hz - min(avg_db_4_8Hz)) / (max(avg_db_4_8Hz) - min(avg_db_4_8Hz)); % Normalize to 0-1
    avg_db_4_8Hz_standardized = 2 * avg_db_4_8Hz_standardized - 1; % Convert to -1 to 1
    ...
    fH = figure('Position', [50, 50, 1200, 800]);
    aH = axes('parent', fH);
    set(aH, 'Position', [0.1, 0.35, 0.8, 0.55]); % Adjust as needed
    colormap("jet");
    ...
    ylabel(aH, 'Hz');
    ptstr = regexprep(num2str(unique([MT.patient])'), '\s+', '/');
   ttlstr = sprintf('%s, %s, %s, %s, %s, p<%1.2f', transtype, desctype, regiontype, ptstr, num2str(walktype), pval);
    title(aH, ttlstr);


    %---------------------------------------------------
    % Plotting the Standardized dB Values for 4-8 Hz Below the Spectrogram
aH2 = axes('parent', fH, 'Position', [0.1, 0.1, 0.745, 0.2]); % Adjust 'Position' values to fit your layout
plot(tsec, avg_db_4_8Hz_standardized, 'k', 'LineWidth', 2);
xlabel(aH2, 'Time (sec)');
ylabel(aH2, 'Standardized dB (4-8 Hz)');
ylim(aH2, [-1, 1]); % Set y-limits to match the standardized range
    %---------------------------------------------------

    saveas(fH, fullfile('C:\Users\burke\OneDrive\Desktop\NeuroIOT\FiguresExample', sprintf('spectrogram_%d.png', StartingSampleID)));
...
%                     clust_info = cellfun(@numel,clustinfo.PixelIdxList);
            ...
            dxx_me = mean(dxx,2,"omitnan");
            dxx_se = std(dxx,0,2,'omitnan');
            % dxx_se = std(dxx,0,2,'omitnan')./sqrt(size(dxx,2));
            % dxx_bl = median(dxx_me);
            plot(aH,obj.MultTrans.TimeSec_xs,(dxx_me)*9+10,'k','LineWidth',2);
            plot(aH,obj.MultTrans.TimeSec_xs,((dxx_me-dxx_se))*9+10,':k','LineWidth',0.5);
            plot(aH,obj.MultTrans.TimeSec_xs,((dxx_me+dxx_se))*9+10,':k','LineWidth',0.5);
            if plottrials
            ...
            if plotboxcomp
                ...
                for k=1:size(uPtCh,1)
                    ...
                    cc_b1 = corrcoef(dx_b1(~nan_idx),pwr_b1(~nan_idx));
                    cc_b2 = corrcoef(dx_b2(~nan_idx),pwr_b2(~nan_idx));
                ...
                boxplot(aH3(1),[pwr_box1,pwr_box2],{'box1 (-.)','box2 (--)'})
                [h,p,ci,stats] = ttest2(pwr_box1,pwr_box2);
                ylabel(aH3(1),'dB')
                title(aH3(1),sprintf('All Trials (p=%0.1e)',p))
                aH3(2) = subplot(1,2,2,'parent',fH3);
                boxplot(aH3(2),[mpwr_box1,mpwr_box2],{'box1 (-.)','box2 (--)'})
                hold(aH3(2),'on');
                ...
                for k=1:length(mpwr_box1)
                    x1 = (rand-0.5)/3+1;
                    x2 = (rand-0.5)/3+2;
                    plot(aH3(2),x1,mpwr_box1(k),'Marker',mrk_style{rgnum(k)},'MarkerFaceColor',mrk_color{ptnum(k)},'MarkerEdgeColor',mrk_color{ptnum(k)},'MarkerSize',mrk_size(rgnum(k)),'LineWidth',2);
                    plot(aH3(2),x2,mpwr_box2(k),'Marker',mrk_style{rgnum(k)},'MarkerFaceColor',mrk_color{ptnum(k)},'MarkerEdgeColor',mrk_color{ptnum(k)},'MarkerSize',mrk_size(rgnum(k)),'LineWidth',2);                    
                    plot(aH3(2),[x1,x2],[mpwr_box1(k),mpwr_box2(k)],'LineStyle',':','Color',mrk_color{ptnum(k)})
                end
                [h,p,ci,stats] = ttest2(mpwr_box1,mpwr_box2);
                ylabel(aH3(2),'Mean dB')
                title(aH3(2),sprintf('Mean Across Patient/Region (p=%0.1e)\nPatient (r=1, g=2, b=3, c=4, m=5)\nRegion (o=anthip, +=lattemp, *=entperi, x=posthip)',p))
                ...
                title(aH4,sprintf('Correlation Across Patient/Region (p=%0.1e)\nPatient (r=1, g=2, b=3, c=4, m=5)\nRegion (o=anthip, +=lattemp, *=entperi, x=posthip)',p))
            end
        ...
        function varargout = plotMultTransSpecDiffPerm(obj,varargin)
            ...
            %Getting trials and specgram data
            [MT1,~,dx1] = obj.filterMultTransData('transtype',transtype{1},'regiontype',regiontype{1},'walktype',walktype{1},'desctype',desctype{1},'patienttype',patienttype{1},'veltype',veltype{1}); %table of all trials and corresponding info
            [pwr1,tsec,freq] = obj.getFilteredMultTransData(MT1,tsamp); %raw power for all trials (time x freq x trial)

            [MT2,~,dx2] = obj.filterMultTransData('transtype',transtype{2},'regiontype',regiontype{2},'walktype',walktype{2},'desctype',desctype{2},'patienttype',patienttype{2},'veltype',veltype{2}); %table of all trials and corresponding info
...
%                 clust_info = cellfun(@numel,clustinfo.PixelIdxList);
            ...
            xlabel(aH,'sec');
            ylabel(aH,'Hz');
            ptstr = [regexprep(num2str(unique([MT1.patient])'),'\s+','/'),'::',regexprep(num2str(unique([MT2.patient])'),'\s+','/')];
            title(aH,sprintf('%s, %s, %s, %s, %s, %s, p<%1.2f',...
                [transtype{1},'::',transtype{2}],...
                [desctype{1},'::',desctype{2}],...
                [regiontype{1},'::',regiontype{2}],ptstr,...
                [num2str(walktype{1}),'::',num2str(walktype{2})],...
                [num2str(ntrials1),'::',num2str(ntrials2)],pval));
